...
export class AnatomyViewer {
  ...
  constructor(container: HTMLElement, callbacks: ViewerCallbacks) {
    ...
    this.renderer.toneMappingExposure = 0.36;
    // Soft contact shadows are the single biggest "real hardware" cue: they
    // ground chips, slots and fin-stacks against each other instead of letting
    // them read as decals on a painted slab. One key light casts, so the extra
    // cost stays at a single depth pass per frame.
    this.renderer.shadowMap.enabled = true;
    this.renderer.shadowMap.type = THREE.PCFSoftShadowMap;
    this.renderer.localClippingEnabled = true;
    // Localised by the React layer via setCanvasLabel once the dictionary is known.
    this.renderer.domElement.setAttribute("aria-label", "Interactive 3D computer hardware model");
  ...
  private buildEnvironment() {
    // Neutral studio rig rather than coloured "lab" light: a warm-white key, a
    // cool-white fill and a near-white rim read as product photography, which is
    // what separates real hardware from a toy-like render.
    this.scene.add(new THREE.AmbientLight(0xffffff, 0.28));
    this.scene.add(new THREE.HemisphereLight(0xffffff, 0x15181c, 0.65));

    const key = new THREE.DirectionalLight(0xfff3e6, 3.5);
    key.position.set(4.8, 6.5, 6.8);
    key.castShadow = true;
    key.shadow.mapSize.set(2048, 2048);
    key.shadow.camera.near = 4;
    key.shadow.camera.far = 22;
    key.shadow.camera.left = -2.9;
    key.shadow.camera.right = 2.9;
    key.shadow.camera.top = 2.9;
    key.shadow.camera.bottom = -2.9;
    ...
    const fill = new THREE.DirectionalLight(0xf0f1f2, 1.1);
    fill.position.set(-4.5, 1.2, 5.2);
    this.scene.add(fill);

    const rim = new THREE.DirectionalLight(0xf2f5f7, 1.6);
    rim.position.set(-4, 3.5, -5.5);
    this.scene.add(rim);

    // Start with the procedural gradient (renders immediately), then upgrade to
    // a real studio HDRI once it loads — photo-based reflections are what make
    // metals stop reading as plastic.
    ...
    this.plinth = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x23282d, roughness: 0.3, metalness: 0.5 }),
    );
    ...
    const grid = new THREE.GridHelper(11, 22, 0x245b64, 0x17333a);
    grid.position.y = PLINTH_TOP - 0.03;
    ...
    this.scene.add(
      new THREE.Points(
        particleGeometry,
        new THREE.PointsMaterial({ color: 0x8a9296, size: 0.014, transparent: true, opacity: 0.16 }),
      ),
    );
  }

  /**
   * A small warm-to-cool gradient probe is replaced here with a proper studio
   * backdrop: a bright softbox up top, cool and warm sidelights, and a dark
   * floor. PMREM bakes it once, so metals pick up moving specular streaks as the
   * model turns — the single biggest readability win for these solid parts.
   */
  private buildEnvironmentMap() {
    ...
    grad.addColorStop(0.34, "#c2c9ce");
    grad.addColorStop(0.5, "#5a6065");
    grad.addColorStop(0.62, "#2a2f33");
    grad.addColorStop(1.0, "#0a0c0e");
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, c.width, c.height);

    const blob = (x: number, y: number, rw: number, rh: number, a: number, col: string) => {
      const rg = ctx.createRadialGradient(x, y, 0, x, y, Math.max(rw, rh));
    ...
    // key softbox, cool fill, warm kicker, faint horizon glow
    blob(c.width * 0.5, c.height * 0.16, 340, 140, 0.95, "#ffffff");
    blob(c.width * 0.84, c.height * 0.3, 190, 120, 0.55, "#e8edf0");
    blob(c.width * 0.14, c.height * 0.32, 150, 95, 0.45, "#f0ede8");
    blob(c.width * 0.5, c.height * 0.5, 520, 42, 0.3, "#b9c1c6");
  ...
  /**
   * Replaces the procedural gradient environment with a CC0 studio HDRI
   * (Poly Haven, `studio_small_08`) once it loads. Real HDR reflections give
   * metals their characteristic moving specular streaks far better than the
   * synthetic gradient ever can.
   */
  private loadStudioEnvironment() {
    new RGBELoader().load(
      ...
      undefined,
      () => {
        // Keep the procedural gradient; the HDRI is an enhancement, not a
        // requirement.
      },
    );
  }
