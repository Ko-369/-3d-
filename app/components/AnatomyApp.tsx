...
type Modal = "lesson" | "notes" | "animation" | "system" | null;
...
function OrganArt({
  ...
  return (
    <img
      key={`${organ.id}-${asset}`}
      src={`/hardware/${organ.id}/${asset}.webp`}
      alt={alt}
      width={size}
      height={size}
...
export function AnatomyApp({ locale, dictionary }: { locale: LocaleConfig; dictionary: Dictionary }) {
  ...
  const [organId, setOrganId] = useState<OrganId>("motherboard");
  ...
  const [favorites, setFavorites] = useState<OrganId[]>(() => readJSON<OrganId[]>(STORAGE_KEYS.favorites, []));
  ...
  const reference = organById[organId === "motherboard" ? "cpu" : "motherboard"];
  const filteredOrgans = useMemo(
    () =>
      organs.filter((item) => {
        const matchesQuery = `${item.name} ${item.system}`
          .toLocaleLowerCase(locale.code)
          .includes(query.toLocaleLowerCase(locale.code));
        const matchesFavorite = !onlyFavorites || favorites.includes(item.id);
        return matchesQuery && matchesFavorite;
      }),
    [organs, query, locale.code, onlyFavorites, favorites],
  );

  const isFavorite = (id: OrganId) => favorites.includes(id);
  ...
  const setNote = (id: OrganId, value: string) => {
    setNotes((prev) => {
  ...
  const selectOrgan = (id: OrganId) => {
    if (organById[id].illustrated) {
      ["organ", "microscopic", "compare", "location"].forEach((asset) => {
        ...
        image.src = `/hardware/${id}/${asset}.webp`;
      });
    }
    setOrganId(id);
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <button className="brand" type="button" onClick={() => selectOrgan("motherboard")} aria-label={t.brand.home}>
          <strong>Computer Architecture Lab<sup>◈</sup></strong>
          <em>{t.brand.tagline}</em>
        </button>
        <nav className="main-nav" aria-label="Primary navigation">
          <button className="active"><Compass size={17} /> {t.nav.explore}</button>
          <a className="nav-assembly" href={`/${locale.code}/assembly`}><Wrench size={17} /> {locale.code === "zh" ? "装机" : "Assemble"}</a>
          ...
          <button onClick={() => setModal("notes")}><NotebookPen size={17} /> {t.nav.notes}</button>
        </nav>
        ...
        <div className="profile-wrap">
          ...
          >
            <span>CS</span><ChevronDown size={15} />
          </button>
          {profileOpen && (
            <>
              <button className="profile-backdrop" aria-hidden onClick={() => setProfileOpen(false)} />
              <div className="profile-menu" role="menu" aria-label={t.profile.open}>
                <div className="profile-stat">
                  <Star size={14} />
                  <span>{isZh ? "已收藏" : "Favorites"}</span>
                  <b>{favorites.length}</b>
                </div>
                <div className="profile-stat">
                  <NotebookPen size={14} />
                  <span>{isZh ? "笔记" : "Notes"}</span>
                  <b>{Object.values(notes).filter(Boolean).length}</b>
                </div>
                <button type="button" className="profile-clear" onClick={clearData}>
                  <Trash2 size={14} /> {isZh ? "清除本地数据" : "Clear local data"}
                </button>
              </div>
            </>
          )}
      ...
      <div className="workspace">
        <aside className={`organ-library ${mobileLibrary ? "open" : ""}`}>
          <div className="panel-heading">
            ...
            <button
              ...
              title={t.library.saved}
              onClick={() => setOnlyFavorites((value) => !value)}
            >
              <Bookmark size={17} fill={onlyFavorites ? "currentColor" : "none"} />
            </button>
          </div>
            ...
            {filteredOrgans.map((item) => (
              ...
              >
                ...
                <span
                  role="button"
                  tabIndex={0}
                  className={`favorite ${isFavorite(item.id) ? "on" : ""}`}
                ...
                >
                  <Bookmark size={14} fill={isFavorite(item.id) ? "currentColor" : "none"} />
                </span>
              </button>
            ))}
          </div>
        ...
        <aside className="info-panel" ref={contentRef}>
          <div className="info-kicker" data-reveal><BrainCircuit size={13} /> {format(t.info.kicker, { organ: organ.name })}</div>
          ...
          <div className="medical-note" data-reveal><BrainCircuit size={16} /><p><b>{t.info.medical}</b>{organ.medical}</p></div>
      ...
      {modal && (
        <LearningModal
          ...
          note={notes[organId] ?? ""}
          onNoteChange={(value) => setNote(organId, value)}
...
/**
 * A staged flow through the organ's labelled parts. The moving dot reads as
 * data / energy travelling through the component — the "how it works" motion
 * that a static image can't show.
 */
function FlowDiagram({ nodes, accent }: { nodes: string[]; accent: string }) {
  const items = nodes.flatMap((label, index) => {
    const node = (
      <div className="flow-node" key={`n-${index}`}>
        <span className="flow-dot" style={{ animationDelay: `${index * 0.35}s` }}>{index + 1}</span>
        <small>{label}</small>
      </div>
    );
    return index === 0 ? [node] : [<i className="flow-link" key={`l-${index}`} />, node];
  });
  return (
    <div className="flow-diagram" style={{ "--flow-accent": accent } as React.CSSProperties}>
      {items}
    </div>
  );
}
...
function LearningModal({
  ...
  const lesson = getLesson(organ.id, localeCode);
  const title =
    type === "notes" ? (localeCode === "zh" ? "我的笔记" : "Notes")
        ...
        {type === "notes" ? (
          <>
            <textarea
              ...
              placeholder={localeCode === "zh" ? "记录这个部件的要点…" : "Write a note about this part…"}
            />
            <p className="notes-hint">{localeCode === "zh" ? "自动保存到本机浏览器" : "Saved automatically to this browser"}</p>
          </>
        ) : type === "system" ? (
          <>
            <p>{format(t.modal.systemIntro, vars)}</p>
            {/* Shown whole rather than cropped into the circular demo — the
                point of this view is the component inside the larger system. */}
          ...
          <>
            <p>{lesson.intro}</p>
            <FlowDiagram nodes={organ.hotspots.map((hotspot) => hotspot.label)} accent={organ.accent} />
            <p className="flow-caption">{organ.function}</p>
            <button className="lesson-button" onClick={onClose}>{t.modal.continueExploring} <ArrowRight size={16} /></button>
          </>
        ) : (
          <>
